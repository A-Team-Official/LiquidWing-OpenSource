...
@ModuleInfo(name = "JelloTabGui", description = "Toggles visibility of the HUD.", category = ModuleCategory.RENDER, array = false, Chinese = "果冻选项卡")
class JelloTabGui : Module() {
    val Shader = BoolValue("Shader", false)
    ...
    fun rendertabGui(evnet: Render2DEvent) {
        ...
        if(selectgradientBackground.get()) {
            VisualUtils.R2DUtils.drawGradientRect(x, y + categoryAnimaiton.y - categorysupery, width + 5, y + categoryAnimaiton.y + 17f - categorysupery,  /*415277420*/
        ...
        if(openModuleGui) {
            ...
            RenderUtils.drawShadow((x + width + 10).toInt().toFloat(), y.toInt().toFloat(), (width + 5).toInt().toFloat(), (positiony + 2f).toInt().toFloat())

            BlurBuffer.CustomBlurArea((x + width + 10), y, (width + 5), (positiony + 2f),30f)
            ...
            if(selectgradientBackground.get()) {
                VisualUtils.R2DUtils.drawGradientRect(x + width + 10, y + moduleAnimaiton.y - modulesupery, x + width + width + 15, y + moduleAnimaiton.y - modulesupery + 17f,
                        Color(tR, tG, tB, 255).rgb,
                        Color(bR, bG, bB, 255).rgb)
            }
        }
        ...
        selecteModule.forEachIndexed{index , module ->
            ...
            val font = if(module.state)
                FontLoaders.J20 else FontLoaders.J20
    ...
    private fun updateBackGound() {
        RenderUtils.drawShadow(x.toInt().toFloat(), y.toInt().toFloat(), width.toInt().toFloat(), (height - 1).toInt().toFloat())
        val hudMod = LiquidBounce.moduleManager.getModule(
                HUD::class.java
        ) as HUD
        fun getClientColor(): Color {
        ...
        BlurBuffer.CustomBlurArea(x, y, width, (height - 1),40f)
        if (Shader.get()) {
            if (colorMode.get() == "Tenacity") {
                val gradientColor1 = RenderUtils.interpolateColorsBackAndForth(
                        15,
