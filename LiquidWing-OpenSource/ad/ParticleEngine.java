...
import net.ccbluex.liquidbounce.utils.GLUtils2;
...
public class ParticleEngine {
    ...
    public void render(float mouseX, float mouseY) {
        ...
        for (Particle p : particles) {
            ...
            GLUtils2.startSmooth();
            RenderUtils.drawBorderedCircle(p.x + MathHelper.sin(p.ticks / 2) * 50 + -xOffset / 5, sr.getScaledHeight() - ((p.ticks * p.speed) * p.ticks / 10 + yOffset / 5), p.radius * (p.opacity / 32), c.getRGB(), c.getRGB());
            GLUtils2.endSmooth();
            p.ticks += (0.9f / (float) Minecraft.getDebugFPS());// +(0.005*1.777*(GLUtils2.getMouseX()-lastMouseX) + 0.005*(GLUtils2.getMouseY()-lastMouseY));
        ...
        GlStateManager.enableBlend();
        GlStateManager.disableAlpha();
        lastMouseX = GLUtils2.getMouseX();
        lastMouseY = GLUtils2.getMouseY();
    }
}
