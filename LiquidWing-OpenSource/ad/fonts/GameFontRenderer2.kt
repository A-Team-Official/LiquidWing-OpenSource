...
class GameFontRenderer2(font: Font) : IWrappedFontRenderer {

    val fontHeight: Int
    var defaultFont = AWTFontRenderer2(font)
    private var boldFont = AWTFontRenderer2(font.deriveFont(Font.BOLD))
    private var italicFont = AWTFontRenderer2(font.deriveFont(Font.ITALIC))
    private var boldItalicFont = AWTFontRenderer2(font.deriveFont(Font.BOLD or Font.ITALIC))
    ...
    override fun drawCenteredStringWithShadow(text: String, x: Float, y: Float, color: Int): Int {
    ...
    fun trimStringToWidth(p_trimStringToWidth_1_: String, p_trimStringToWidth_2_: Int): String {
        return this.trimStringToWidth(p_trimStringToWidth_1_, p_trimStringToWidth_2_, false)
    }

    fun trimStringToWidth(
        p_trimStringToWidth_1_: String,
        p_trimStringToWidth_2_: Int,
        p_trimStringToWidth_3_: Boolean
    ): String {
        val stringbuilder = StringBuilder()
        var i = 0
        val j = if (p_trimStringToWidth_3_) p_trimStringToWidth_1_.length - 1 else 0
        val k = if (p_trimStringToWidth_3_) -1 else 1
        var flag = false
        var flag1 = false
        var l = j
        while (l >= 0 && l < p_trimStringToWidth_1_.length && (i < p_trimStringToWidth_2_)) {
            val c0 = p_trimStringToWidth_1_[l]
            val i1 = getCharWidth(c0)
            if (flag) {
                flag = false
                if (c0 != 'l' && c0 != 'L') {
                    if (c0 == 'r' || c0 == 'R') {
