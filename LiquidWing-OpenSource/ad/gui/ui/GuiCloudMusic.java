...
import ad.gui.utils.CloudMusicAPI;
import ad.gui.utils.Track;
...
public class GuiCloudMusic extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (Math.ceil(sidebarAnimation) > 1) {
            ...
            RenderUtils.drawRoundedRect(newX - 10, y, newWidth, y + height, 2, new Color(255,255,255).getRGB());

            //歌单导入输入框
            textField.draw(newX + 6, y + 2);
            RenderUtils.drawRoundedRect(newWidth - 26, y + 5, newWidth - 7, y + 17, 2, RenderUtils.isHovering(mouseX, mouseY, newWidth - 26, y + 5, newWidth - 7, y + 17) || MusicManager.INSTANCE.analyzeThread != null ? new Color(235,20,20).getRGB() : new Color(255,40,40).getRGB());
            FontLoaders.F14.drawString("导入", newWidth - 23f, y + 8f, new Color(240, 240, 240).getRGB());

            if (textField.textString.isEmpty()) {
                FontLoaders.F14.drawString("输入歌单ID", newX + 8, y + 8f, Color.WHITE.getRGB());
            }
            ...
            this.scrollAni = RenderUtils.getAnimationState(this.scrollAni, scrollY, Math.max(10, (Math.abs(this.scrollAni - (scrollY))) * 50) * 0.3f);
        ...
        //主框架
        RenderUtils.drawRoundedRect(x, y, x + width, y + height, 2, new Color(245,245,245).getRGB());
        RenderUtils.drawRoundedRect(x, y + height - 60, x + width, y + height, 2, new Color(230,230,230).getRGB());
        RenderUtils.drawRect(x, y + height - 60, x + width, y + height - 58, new Color(230,230,230).getRGB());

        FontLoaders.F16.drawString("网易云音乐", x + 15, y + 8, Color.BLACK.getRGB());
        ...
        //音量
        RenderUtils.drawRoundedRect(x + 10, y + 24, x + width - 10, y + 28, 1.4f, Color.GRAY.getRGB());
        RenderUtils.drawRoundedRect(x + 10, y + 24, x + 10 + (130 * volume), y + 28, 1.4f, new Color(255,40,40).getRGB());
        RenderUtils.circle(x + 10 + (130 * volume), y + 26, 3, new Color(255,40,40).getRGB());

        if (Mouse.isButtonDown(0) && RenderUtils.isHovering(mouseX, mouseY, x + 10, y + 21, x + width - 10, y + 32)) {
        ...
        if (MusicManager.INSTANCE.loadingThread != null) {
            ...
            RenderUtils.circle(x + 10 + (1.3f * MusicManager.INSTANCE.downloadProgress), y + height - 48, 3, Palette.fade(new Color(255, 50, 50, 255)).getRGB());
            RenderUtils.circle(x + 10 + (1.3f * MusicManager.INSTANCE.downloadProgress), y + height - 48, 2, Color.WHITE.getRGB());
        } else {
            RenderUtils.drawRoundedRect(x + 10, y + height - 50, x + 10 + (1.3f * progress), y + height - 46, 1.4f, new Color(255,40,40).getRGB());
            RenderUtils.circle(x + 10 + (1.3f * progress), y + height - 48, 3, new Color(255,40,40).getRGB());
            RenderUtils.circle(x + 10 + (1.3f * progress), y + height - 48, 2, new Color(255,255,255).getRGB());
        }

        //按钮
        RenderUtils.circle(x + (width / 2), y + height - 24, 12, new Color(255,40,40).brighter().getRGB()); //播放和暂停

        Fonts.micon15.drawString("R", x + 5, y + 8.5f, Color.BLACK.getRGB());
        ...
        FontLoaders.F16.drawString(songName, x + (width / 2) - (FontLoaders.F16.getStringWidth(songName) / 2f) - 1.5f, y + (height / 2 - 90), Color.BLACK.getRGB());
        FontLoaders.F14.drawString(songArtist, x + (width / 2) - (FontLoaders.F14.getStringWidth(songArtist) / 2f) - 1.5f, y + (height / 2 - 82), Color.BLACK.getRGB());
        GL11.glDisable(GL11.GL_SCISSOR_TEST);

        if (MusicManager.INSTANCE.getMediaPlayer() != null) {

            if (MusicManager.INSTANCE.getMediaPlayer().getStatus() == Status.PLAYING) {
                Fonts.micon30.drawString("K", x + (width / 2) - (Fonts.micon30.getStringWidth("K") / 2f), y + height - 23, Color.WHITE.getRGB());
            } else {
                Fonts.micon30.drawString("J", x + (width / 2) - (Fonts.micon30.getStringWidth("J") / 2f) + 1, y + height - 23, Color.WHITE.getRGB());
            }

        } else {
            Fonts.micon30.drawString("J", x + (width / 2) - (Fonts.micon30.getStringWidth("J") / 2f) + 1, y + height - 23f, Color.WHITE.getRGB());
        }

        Fonts.micon30.drawString("L", x + width / 2 - (Fonts.micon30.getStringWidth("L") / 2f) - 30, y + height - 23, Color.BLACK.getRGB());
        Fonts.micon30.drawString("M", x + width / 2 - (Fonts.micon30.getStringWidth("M") / 2f) + 27.5f, y + height - 23, Color.BLACK.getRGB());

        if (MusicManager.INSTANCE.repeat) {
            Fonts.micon15.drawString("O", x + width - 20, y + height - 26f, Color.BLACK.getRGB());
        } else {
            Fonts.micon15.drawString("N", x + width - 20, y + height - 26f, new Color(50,50,50).getRGB());
        }

        if (MusicManager.INSTANCE.lyric) {
            Fonts.micon15.drawString("P", x + 10, y + height - 26f, Color.BLACK.getRGB());
        } else {
            Fonts.micon15.drawString("P", x + 10, y + height - 26f, new Color(50,50,50).getRGB());
        }

        if (MusicManager.INSTANCE.currentTrack != null) {
            if (MusicManager.INSTANCE.getArt(MusicManager.INSTANCE.currentTrack.id) != null) {
                GL11.glPushMatrix();
                RenderUtils.drawImage(MusicManager.INSTANCE.getArt(MusicManager.INSTANCE.currentTrack.id), x + (width / 2) - 50, y + (height / 2 - 10) - 50, 100, 100, 1f);
                GL11.glPopMatrix();
            }
        }
