...
public class QRLoginScreen extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        FontLoaders.F16.drawString(text, res.getScaledWidth() / 2f - (FontLoaders.F16.getStringWidth(text) / 2f), 30, 0xFFFFFFFF);
        FontLoaders.F16.drawString(state, res.getScaledWidth() / 2f - (FontLoaders.F16.getStringWidth(state.replaceAll("\247.", "")) / 2f), 50, 0xFFFFFFFF);

        if(this.loginProcessThread != null) {
            RenderUtils.drawImage(new ResourceLocation("cloudMusicCache/qrcode"), res.getScaledWidth() / 2f - 64, 70, 128, 128, 1f);
        }
