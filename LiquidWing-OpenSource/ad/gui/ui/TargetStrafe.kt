...
@ModuleInfo(name = "TargetStrafe",description = "Strafe around your target. (Require Fly or Speed to be enabled)", category = ModuleCategory.MOVEMENT, Chinese = "锁定目标灵活移动")
class TargetStrafe : Module() {
    ...
    fun strafe(event: MoveEvent, moveSpeed: Double) {
        ...
        val rotYaw = RotationUtils.getRotationsEntity(target.unwrap()).yaw

        val forward = if (mc.thePlayer!!.getDistanceToEntity(target) <= radius.get()) 0.0 else 1.0
        val strafe = direction.toDouble()
        var modifySpeed = if (expMode.get()) maximizeSpeed(target.unwrap(), moveSpeed, killAura.rangeValue.get()) else moveSpeed
    ...
    private fun maximizeSpeed(ent: EntityLivingBase, speed: Double, range: Float): Double {
        ...
        return if (mc2.player!!.getDistance(ent as Entity) <= radius.get()) speed.coerceIn(0.0, range.toDouble() / 20.0) else speed
    }
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (( alwaysRender.get()) && render.get()) {
            target?:return
            GL11.glPushMatrix()
            GL11.glTranslated(
            ...
            if (outLine.get()) {
                GL11.glLineWidth(thicknessValue.get() + 5.25F)
            ...
            for (i in 0..360 step 60 - accuracyValue.get()) { // You can change circle accuracy  (60 - accuracy)
                when (colorType.get()) {
                    ...
                    "Dynamic" ->  GL11.glColor4f(0.25f, 1f, 0.25f, 1f)
                    "Rainbow" -> {
