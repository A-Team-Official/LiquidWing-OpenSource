...
@ModuleInfo(name = "LongJump", description = "Allows you to jump further.", category = ModuleCategory.MOVEMENT, Chinese = "远距离跳跃")
class LongJump : Module() {
    private val modeValue = ListValue("Mode", arrayOf("NCP", "AACv1", "AACv2", "AACv3", "Mineplex", "Mineplex2", "Mineplex3","hyt"), "hyt")
            ...
            run {
                when (mode.toLowerCase()) {
                    ...
                    "hyt" -> {
                        ...
                        thePlayer.jumpMovementFactor = 0.03f
                        MovementUtils.strafe(MovementUtils.speed * if (canBoost) ncpBoostValue.get() else 1f)
                        canBoost = false
                        ...
                        if (thePlayer.fallDistance > 1.5f) {
                            ...
                            thePlayer.motionX = (-3.5).toDouble()
                            mc.timer.timerSpeed = 0.3f
                        }
                    }
                    "mineplex" -> {
    ...
    override fun onEnable() {
        var timer = moduleManager.getModule(
            Timer::class.java
        ) as Timer
    ...
    fun onMove(event: MoveEvent) {
        ...
        else if (mode.equals("hyt", ignoreCase = true) && !MovementUtils.isMoving && jumped) {
