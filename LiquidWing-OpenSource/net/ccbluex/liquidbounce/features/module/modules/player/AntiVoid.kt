...
@ModuleInfo(name = "AntiVoid", category = ModuleCategory.PLAYER, description = "null", Chinese = "虚空回弹")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when (modeValue.get().toLowerCase()) {
            ...
            "packetflag" -> {
                if (!voidOnlyValue.get() || checkVoid()) {
                    if (mc.thePlayer!!.fallDistance > maxFallDistValue.get() && !tried) {
                        mc2.connection!!.sendPacket(CPacketPlayer.Position(mc.thePlayer!!.posX + 1, mc.thePlayer!!.posY + 1, mc.thePlayer!!.posZ + 1, false))
                        tried = true
                    }
                }
            }
            ...
            "oldcubecraft" -> {
                ...
                if (!voidOnlyValue.get() || checkVoid()) {
                    if (mc.thePlayer!!.fallDistance> maxFallDistValue.get() && mc.thePlayer!!.posY <lastRecY + 0.01 && mc.thePlayer!!.motionY <= 0 && !mc.thePlayer!!.onGround && !flagged) {
                        ...
                        if (!tried) {
                            tried = true
                            mc2.connection!!.sendPacket(CPacketPlayer.Position(mc.thePlayer!!.posX, (32000.0).toDouble(), mc.thePlayer!!.posZ, false))
                        }
                    }
                }
                lastRecY = mc.thePlayer!!.posY
