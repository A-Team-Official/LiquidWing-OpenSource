...
@ModuleInfo(name = "AsianHat",  description = "Yep. China Hat.", category = ModuleCategory.RENDER, Chinese = "帽子")
public class AsianHat extends Module {
    ...
    private final IntegerValue colorAlphaValue = new IntegerValue("Alpha", 0, 0, 255);
    private final IntegerValue colorEndAlphaValue = new IntegerValue("EndAlpha", 156, 0, 255);
    private final FloatValue saturationValue = new FloatValue("Saturation", 0.45F, 0F, 1F);
    private final FloatValue brightnessValue = new FloatValue("Brightness", 1F, 0F, 1F);
    private final IntegerValue mixerSecondsValue = new IntegerValue("Seconds", 2, 1, 10);
    private final IntegerValue spaceValue = new IntegerValue("Color-Space", 6, 0, 100);
    ...
    public void onRender3D(Render3DEvent event) {
        IEntityLivingBase entity = mc.getThePlayer();
        if (entity == null || (noFirstPerson.get() && mc.getGameSettings().getThirdPersonView() == 0)) return;

        final IAxisAlignedBB bb = entity.getEntityBoundingBox();
        double radius = bb.getMaxX() - bb.getMinX();
        double height = bb.getMaxY() - bb.getMinY();
        double posX = entity.getLastTickPosX() + (entity.getPosX() - entity.getLastTickPosX()) * mc.getTimer().getRenderPartialTicks();
        double posY = entity.getLastTickPosY() + (entity.getPosY() - entity.getLastTickPosY()) * mc.getTimer().getRenderPartialTicks();
        double posZ = entity.getLastTickPosZ() + (entity.getPosZ() - entity.getLastTickPosZ()) * mc.getTimer().getRenderPartialTicks();
...
*/
    ...
    public final Color getColor(final IEntity ent, final int index) {
