...
@ModuleInfo(name = "Crosshair", category = ModuleCategory.RENDER, description = "233", Chinese = "自定义准星")
class Crosshair : Module() {
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        GL11.glPushMatrix()
        RenderUtils.drawBorderedRect(sr.scaledWidth / 2f - width, sr.scaledHeight / 2f - gap - size -0, sr.scaledWidth / 2f + 1.0f + width, sr.scaledHeight / 2f - gap -  0, 0.5f, Color(0, 0, 0).rgb, crosshairColor.rgb)
        RenderUtils.drawBorderedRect(sr.scaledWidth / 2f - width, sr.scaledHeight / 2f + gap + 1 +  0 - 0.15f, sr.scaledWidth / 2f + 1.0f + width, sr.scaledHeight / 2f + 1 + gap + size +  0 - 0.15f, 0.5f, Color(0, 0, 0).rgb, crosshairColor.rgb)
        RenderUtils.drawBorderedRect(sr.scaledWidth / 2f - gap - size - 0 + 0.15f, sr.scaledHeight / 2f - width, sr.scaledWidth / 2f - gap - 0 + 0.15f, sr.scaledHeight / 2 + 1.0f + width, 0.5f, Color(0, 0, 0).rgb, crosshairColor.rgb)
        RenderUtils.drawBorderedRect(sr.scaledWidth / 2f + 1 + gap +  0, sr.scaledHeight / 2f - width, sr.scaledWidth / 2f + size + gap + 1.0f + 0, sr.scaledHeight / 2 + 1.0f + width, 0.5f, Color(0, 0, 0).rgb, crosshairColor.rgb)
        GL11.glPopMatrix()
        ...
        get() =
            when (colorModeValue.get().toLowerCase()) {
                ...
                "novoline" -> RenderUtils.getGradientOffset(
                    ...
                    Color(CustomUI.r2.get(), CustomUI.g2.get(), CustomUI.b2.get()),114514.0
                )
                else -> Color.WHITE
            }
}