...
@ModuleInfo(name = "NameTags", description = "Changes the scale of the nametags so you can always read them.", category = ModuleCategory.RENDER, Chinese = "玩家标签")
class NameTags : Module() {
    ...
    private fun renderNameTag(entity: IEntityLivingBase, tag: String) {
        ...
        val width = Fonts.misans40.getStringWidth(text) * 0.5f
        ...
        when (modeValue.get().toLowerCase()) {
            "simple" -> {
                ...
                val width = FontLoaders.nt18.getStringWidth(tag).coerceAtLeast(30) / 2
                val maxWidth = width * 2 + 12F

                quickDrawRect((
                    -width - 6F),
                    -Fonts.newtenacity40.fontHeight * 1.7F,
                    width + 6F,
                    -2F,
                ...
                FontLoaders.nt18.drawStringWithShadow(
                    tag,
                    (-FontLoaders.nt18.getStringWidth(tag) * 0.5F).toInt().toDouble() + 1,
                    (-Fonts.newtenacity40.fontHeight * 1.4F).toInt().toDouble(),
                    Color.WHITE.rgb
            ...
            "liquid" -> {
                if (borderValue.get())
                    quickDrawBorderedRect(
                        ...
                        -2F,
                        width + 4F,
                        Fonts.newtenacity40.fontHeight + 2F,
                        2F,
                        Color(255, 255, 255, 90).rgb,
                        Integer.MIN_VALUE
                    )
                else
                    quickDrawRect(-width - 2F, -2F, width + 4F, Fonts.newtenacity40.fontHeight + 2F, Integer.MIN_VALUE)

                glEnable(GL_TEXTURE_2D)

                Fonts.newtenacity40.drawString(
                    text, 1F + -width, if (Fonts.newtenacity40 == Fonts.newtenacity40) 1F else 1.5F,
                    0xFFFFFF, true
                )
            ...
            "jello" -> {
                ...
                val width = FontLoaders.F18.getStringWidth(tag) / 2
                val maxWidth = (width + 4F) - (-width - 4F)
                var healthPercent = entity.health / entity.maxHealth

                // render bg

                quickDrawRect(-width - 4F, -Fonts.newtenacity40.fontHeight * 3F, width + 4F, -3F, bgColor.rgb)
                ...
                Fonts.newtenacity40.drawString(
                    ...
                    -width * 2,
                    -Fonts.newtenacity40.fontHeight * 2,
                    Color.WHITE.rgb
                )
