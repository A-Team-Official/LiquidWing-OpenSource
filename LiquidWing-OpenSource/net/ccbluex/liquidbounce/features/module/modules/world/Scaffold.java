...
@ModuleInfo(name = "Scaffold", description = "Automatically places blocks beneath your feet.", category = ModuleCategory.WORLD, keyBind = Keyboard.KEY_G, Chinese = "自动搭路")
public class Scaffold extends Module {
    ...
    // Mode
    public final ListValue modeValue = new ListValue("Mode", new String[]{"Test", "Rewinside", "Expand"}, "Test");
    // Basic stuff
    ...
    private final BoolValue placeableDelay = new BoolValue("PlaceableDelay", false);    private final IntegerValue minDelayValue = new IntegerValue("MinDelay", 0, 0, 1000) {
        @Override
        protected void onChanged(final Integer oldValue, final Integer newValue) {
    ...
    private final FloatValue zitterStrength = new FloatValue("ZitterStrength", 0.072F, 0.05F, 0.2F);    private final FloatValue minTurnSpeedValue = new FloatValue("MinTurnSpeed", 180, 1, 180) {
        @Override
        protected void onChanged(final Float oldValue, final Float newValue) {
    ...
    private final IntegerValue markR = new IntegerValue("MarkRed", 255, 0, 255);
    private final IntegerValue markG = new IntegerValue("MarkGreen", 255, 0, 255);
    private final IntegerValue markB = new IntegerValue("MarkBlur", 255, 0, 255);
    private final IntegerValue markA = new IntegerValue("MarkAlpha", 100, 0, 255);
    ...
    public void shader(final BlurEvent event) {
        ...
        if (getBlocksAmount() == 0)
            ...
            if (blockCounter.get() == "Rise") {
                ...
                RenderUtils.drawRoundedRect2((sr.getScaledWidth() / 2F) - 15, height, 30, 35, 0, new Color(0, 0, 0, 255).getRGB());
                GL11.glPopMatrix();
            }
        if (blockCounter.get() == "Novoline") {
            progress = (float) (System.currentTimeMillis() - lastMS) / 600F;
            if (progress >= 1) progress = 1;
            double progress1 = EaseUtils.INSTANCE.easeOutBack(progress);
            GL11.glPushMatrix();
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        int infoWidth = Fonts.tenacitybold.tenacitybold18.tenacitybold18.stringWidth(info);
        ...
        if (blockCounter.get() == ("MC")) {
            ...
            if (itemStack != null) {
                ...
                Fonts.tenacitybold.tenacitybold18.tenacitybold18.drawCenteredString("?", sr.getScaledWidth() / 2F + 0.5F, height + 6, -1);
        }

        if (blockCounter.get() == ("Rise")) {
            GlStateManager.resetColor();
            final int height = sr.getScaledHeight() - 120;
            ShadowUtils.shadow(8f, () -> {
                GL11.glPushMatrix();
                RenderUtils.drawRoundedRect2((sr.getScaledWidth() / 2F) - 50, height - 10, 83, 25, 3, new Color(0, 0, 0).getRGB());
                GL11.glPopMatrix();
            ...
            BlurBuffer.blurRoundArea((sr.getScaledWidth() / 2F) - 50, height - 10, 83, 25,3);

            RenderUtils.drawRoundedRect2((sr.getScaledWidth() / 2F) - 50, height - 10, 83, 25, 3, new Color(0, 0, 0, 80).getRGB());


//            RenderUtils.drawShadow((sr.getScaledWidth() / 2F) - 15, height, 30, 35);
            if (itemStack != null) {
                if (CustomUI.Chinese.get()){
                FontLoaders.F20.drawCenteredString(String.valueOf("方块数量:" + ChatFormatting.YELLOW +getBlocksAmount()), sr.getScaledWidth() / 2F + 6f, height -1.5f, -1,true);}
                else {
                    net.ccbluex.liquidbounce.ui.font.Fonts.pop40.drawCenteredString(String.valueOf("Amount: " + ChatFormatting.YELLOW +getBlocksAmount()), sr.getScaledWidth() / 2F, height -1.5f, -1,true);
                }
                GlStateManager.pushMatrix();
                GlStateManager.enableRescaleNormal();
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                RenderHelper.enableGUIStandardItemLighting();
                mc2.getRenderItem().renderItemAndEffectIntoGUI(itemStack, (int) (sr.getScaledWidth() / 2F - 45), height - 6);
                GlStateManager.disableRescaleNormal();
                GlStateManager.disableBlend();
                RenderHelper.disableStandardItemLighting();
                GlStateManager.popMatrix();
            } else {
                net.ccbluex.liquidbounce.ui.font.Fonts.pop35.drawCenteredString("?", sr.getScaledWidth() / 2F + 0.5F, height - 54, -1,true);
            }
            GlStateManager.resetColor();
            GL11.glPopMatrix();
        }
        if (blockCounter.get() == ("Novoline")) {
            GlStateManager.resetColor();
            final int height = sr.getScaledHeight() - 130;
            double progress1 = EaseUtils.INSTANCE.easeOutBack(progress);
            GL11.glPushMatrix();
            GL11.glTranslated((((sr.getScaledWidth() / 2F)) - 15 +15) * (1 - progress1),(height + 17.5) * (1 - progress1),0);
            GL11.glScaled(progress1, progress1, progress1);
            RenderUtils.drawShadow((sr.getScaledWidth() / 2F) - 15, height, 30, 35);
            RenderUtils.drawShadow((sr.getScaledWidth() / 2F) - 15, height, 30, 35);
            RenderUtils.drawRoundedRect2((sr.getScaledWidth() / 2F) - 15, height, 30, 35, 0, new Color(0, 0, 0, 107).getRGB());
            if (itemStack != null) {
                net.ccbluex.liquidbounce.ui.font.Fonts.pop35.drawCenteredString(String.valueOf(getBlocksAmount()), sr.getScaledWidth() / 2F, height + 40 - 17, -1,true);
                GlStateManager.pushMatrix();
                GlStateManager.enableRescaleNormal();
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                ...
                net.ccbluex.liquidbounce.ui.font.Fonts.pop35.drawCenteredString("?", sr.getScaledWidth() / 2F + 0.5F, height + 6, -1,true);
            }
            GlStateManager.resetColor();
            GL11.glPopMatrix();
        }
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        for (int i = 0; i < (modeValue.get().equalsIgnoreCase("Expand") ? expandLengthValue.get() + 1 : 2); i++) {
            ...
            if (BlockUtils.isReplaceable(blockPos) && placeInfo != null) {
                RenderUtils.drawBlockBox(blockPos, new Color(markR.get(), markG.get(), markB.get(), markA.get()), false);
                break;
            }
        }
    }
