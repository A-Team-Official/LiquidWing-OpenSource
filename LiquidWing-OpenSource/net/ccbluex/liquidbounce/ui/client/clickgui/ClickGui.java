...
public class ClickGui extends WrappedGuiScreen {
    ...
    private final IResourceLocation hudIcon = classProvider.createResourceLocation("liquidwing" + "/custom_hud_icon.png");
    ...
    public ClickGui() {
        ...
        yPos += 20;
        panels.add(new Panel("Config", 100, yPos, width, height, false) {
            @Override
            public void setupItems() {
                for (File config : LiquidBounce.fileManager.configsDir.listFiles()){
                    String name = config.getName();
                    getElements().add(new ButtonElement(name) {
                        @Override
