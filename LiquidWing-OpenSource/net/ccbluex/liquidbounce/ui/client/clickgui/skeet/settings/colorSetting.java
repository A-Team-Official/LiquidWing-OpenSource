...
public class colorSetting extends Downward<ColorValue> {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        //绘制调色板SKID
        int black = RenderUtils.getColor(0);
        ...
        if (colorValue.isExpanded()) {
            ...
            RenderUtils.drawBorderedRect(expandedX, expandedY,expandedX + expandedWidth, expandedY+expandedHeight + 70,1, new Color(0,0,0,0).getRGB(),new Color(5,23,37).getRGB());
            ...
            while ((float)i2 < 5.0f) {
                ...
                RenderUtils.drawGradientRect(hueSliderLeft, asLeft, hueSliderRight, asLeft + alphaSelectorX, RenderUtils.getColor(Color.HSBtoRGB(1.0f - 0.2f * (float)i2, 1.0f, 1.0f)), RenderUtils.getColor(Color.HSBtoRGB(1.0f - 0.2f * (float)(i2 + 1), 1.0f, 1.0f)));
                if (!last) {
    ...
    private void drawColorPickerRect(float left, float top, float right, float bottom) {
        int hueBasedColor = RenderUtils.getColor(Color.HSBtoRGB(this.hue, 1.0F, 1.0F));
