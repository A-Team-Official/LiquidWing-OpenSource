...
class EditorPanel(private val hudDesigner: GuiHudDesigner, var x: Int, var y: Int) : MinecraftInstance() {
    ...
    fun drawPanel(mouseX: Int, mouseY: Int, wheel: Int) {
        ...
        // Scrolling end
        if (shouldScroll) {
            RenderUtils.drawRoundedCornerRect((x + width - 5).toFloat(), (y + 15).toFloat(), (x + width - 2).toFloat(), (y + 197).toFloat(),
                    3f,Color(41, 41, 41).rgb)

            val v = 197 * (-scroll / (realHeight - 170F))
            RenderUtils.drawRoundedRect(x + width - 5F, y + 15 + v, x + width - 2F, y + 20 + v,5f,
    ...
    private fun drawCreate(mouseX: Int, mouseY: Int) {
        ...
        for (element in elements) {
            ...
            Fonts.pop35.drawString(name, x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)

            val stringWidth = Fonts.pop35.getStringWidth(name)
        ...
        Fonts.pop35.drawString("§lCreate element", x + 2F, y + 3.5F, Color.WHITE.rgb)
    }
    ...
    private fun drawSelection(mouseX: Int, mouseY: Int) {
        ...
        Fonts.pop35.drawString("§lCreate element", x + 2, y + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height &&
            mouseY <= y + height + 10) {
            create = true
        }
        ...
        Fonts.pop35.drawString("§lReset", x + 2, y + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height &&
            mouseY <= y + height + 10) {
        ...
        Fonts.pop35.drawString("§lAvailable Elements", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        for (element in LiquidBounce.hud.elements) {
            Fonts.pop35.drawString(element.name, x + 2, y + height, Color.WHITE.rgb)

            val stringWidth = Fonts.pop35.getStringWidth(element.name)
            if (width < stringWidth + 8) {
                width = stringWidth + 8
            }
        ...
        Fonts.pop35.drawString("§lEditor", x + 2F, y + 3.5f, Color.WHITE.rgb)
    }
    /**
     * Draw editor panel
     */
    private fun drawEditor(mouseX: Int, mouseY: Int) {
        ...
        // X
        Fonts.pop35.drawString("X: ${"%.2f".format(element.renderX)} (${"%.2f".format(element.x)})", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Y
        Fonts.pop35.drawString("Y: ${"%.2f".format(element.renderY)} (${"%.2f".format(element.y)})", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Scale
        Fonts.pop35.drawString("Scale: ${"%.2f".format(element.scale)}", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Horizontal
        Fonts.pop35.drawString("H:", x + 2, y + height, Color.WHITE.rgb)
        Fonts.pop35.drawString(element.side.horizontal.sideName,
        ...
        // Vertical
        Fonts.pop35.drawString("V:", x + 2, y + height, Color.WHITE.rgb)
        Fonts.pop35.drawString(element.side.vertical.sideName,
                x + 12, y + height, Color.GRAY.rgb)
        ...
        for (value in element.values) {
            when (value) {
                is BoolValue -> {
                    // Title
                    Fonts.pop35.drawString(value.name, x + 2, y + height, if (value.get()) Color.WHITE.rgb else Color.GRAY.rgb)

                    val stringWidth = Fonts.pop35.getStringWidth(value.name)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is FloatValue -> {
                    ...
                    Fonts.pop35.drawString(text, x + 2, y + height, Color.WHITE.rgb)

                    val stringWidth = Fonts.pop35.getStringWidth(text)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8

                    // Slider
                    RenderUtils.drawRoundedCornerRect(x + 8F, y + height + 12F, x + prevWidth - 8F, y + height + 13F, 3f,Color(255,255,255).rgb)

                    // Slider mark
                    val sliderValue = x + ((prevWidth - 18F) * (current - min) / (max - min))
                    RenderUtils.drawRoundedCornerRect(8F + sliderValue, y + height + 9F, sliderValue + 11F, y + height
                            + 15F, 3f,Color(37, 126, 255).rgb)
                ...
                is ListValue -> {
                    // Title
                    Fonts.pop35.drawString(value.name, x + 2, y + height, Color.WHITE.rgb)
                    ...
                    for (s in value.values) {
                        // Value title
                        val text = "§c> §r$s"
                        Fonts.pop35.drawString(text, x + 2, y + height, if (s == value.get()) Color.WHITE.rgb else Color.GRAY.rgb)
        ...
        Fonts.pop35.drawString("§l${element.name}", x + 2F, y + 3.5F, Color.WHITE.rgb)

        // Delete button
        if (!element.info.force) {
            val deleteWidth = x + width - Fonts.pop35.getStringWidth("§lDelete") - 2F
            Fonts.pop35.drawString("§lDelete", deleteWidth, y + 3.5F, Color.WHITE.rgb)
