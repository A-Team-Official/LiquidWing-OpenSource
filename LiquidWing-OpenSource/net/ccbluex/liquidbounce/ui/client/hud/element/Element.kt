 ...
 * Border of element
 */
data class Border(val x: Float, val y: Float, val x2: Float, val y2: Float,val radius : Float) {
    ...
    fun draw(x: Double,y: Double,mouseX: Int, mouseY: Int){
        ...
        if (RenderUtils.isHovered(x.toFloat(), y.toFloat(), x2, y2,mouseX, mouseY)){
            RenderUtils.drawOutlinedRoundedRect(this.x.toDouble(),this.y.toDouble(),x2.toDouble(),y2.toDouble(),radius.toDouble(),.5F,Color.WHITE.rgb)
        GlStateManager.resetColor()
        }
    }
}