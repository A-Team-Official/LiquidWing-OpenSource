...
class Radar(x: Double = 5.0, y: Double = 130.0) : Element(x, y) {
    ...
    private val blurstrength = FloatValue("BlurStrength",10f,0f,50f)
    private val shadowValue= FloatValue("ShadowStrength",10f,0f,50f)
    private val radius = FloatValue("Radius", 0F, 0f, 25F)
    ...
    override fun drawElement(): Border? {
        ...
        ShadowUtils.shadow(shadowValue.get(),{
            ...
            RoundedUtil.drawRound(0F, 0F, size, size,radius.get(), Color(backgroundRedValue.get(), backgroundGreenValue.get(),
        ...
        if (blurValue.get()) {
            glTranslated(-renderX, -renderY, 0.0)
            glPushMatrix()
            BlurBuffer.CustomBlurRoundArea(floatX, floatY, size,size, radius.get(),blurstrength.get())
            glPopMatrix()
            glTranslated(renderX, renderY, 0.0)
        }
        ...
        for (entity in mc.theWorld!!.loadedEntityList) {
            if (entity != null && entity !== mc.thePlayer && EntityUtils.isSelected(entity, false)) {
                ...
                BufferBuilder.pos(((positionRelativeToPlayer.x / viewDistance) * size).toDouble(),
        ...
        return Border(0F, 0F, size, size,radius.get())
    }
