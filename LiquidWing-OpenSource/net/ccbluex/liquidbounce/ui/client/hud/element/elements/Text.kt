...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    companion object {
        ...
        fun defaultClient(): Text {
            ...
            text.displayString.set( "LiquidWing | Fps:%fps% | %serverip%")
    ...
    private val TopAplha = IntegerValue("Top-Alpha", 100, 0, 255)
    ...
    private var fontValue = FontValue("Font", Fonts.rise40)
    ...
    private val display: String
        get() {
            val textContent = if (displayString.get().isEmpty() && !editMode)
                "LiquidWing | User:%user% | Fps:%fps% | %serverip%"
            else
                displayString.get()
...
//    private fun drawMarqueeLine(x: Float, y: Float, x2: Float, y2: Float, linewidth: Float, color: Int, color2: Int) {
//        glEnable(GL_BLEND)
...
//            "worldTime" -> return SessionUtils.getFormatWorldTime()
    ...
    override fun drawElement(): Border? {
        ...
        val  mwc1 = Color(CustomUI.r.get(),CustomUI.g.get(),CustomUI.b.get()).rgb
        val mwc2 = Color(CustomUI.r2.get(),CustomUI.g2.get(),CustomUI.b2.get()).rgb
        if (this.Mode.get().equals("Top")) {
            when (shadowValue.get()) {
                "Basic" ->   RenderUtils.drawShadow(-2.5f, -2.5f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat()+ 2f)
                "Thick" -> {
                    RenderUtils.drawShadow(-2f, -2f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat()+ 2f)
                    RenderUtils.drawShadow(-2f, -2f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat()+ 2f)
                }
            }
            if (blurValue.get()){
                ...
                GL11.glPushMatrix()
                BlurBuffer.blurRoundArea(renderX.toFloat(), renderY.toFloat(), (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat() - 0.3f
                    , radiusValue.get().toInt())
                GL11.glPopMatrix()
                GL11.glTranslated(renderX, renderY, 0.0)
            }
            RoundedUtil.drawRound(-2f, -3f, (fontRenderer.getStringWidth(displayText) + 2).toFloat(), fontRenderer.fontHeight.toFloat() + 3f, 0F, Color(0,0,0,TopAplha.get()))
            RenderUtils.drawGradientSideways(-2.0,-3.0, (fontRenderer.getStringWidth(displayText) + 2).toDouble() - 1.6, -2.3, mwc1, mwc2)


        }

        if (this.Mode.get().equals("Onetap")) {
            Gui.drawRect(-2,-5, fontRenderer.getStringWidth(displayText) + 2, fontRenderer.fontHeight, Color(0,0,0,onealpha.get()).rgb)
                ...
                for (i in 0..(amountValue.get()-1)) {
                    ...
                    RenderUtils.drawGradientSideways(-1.4 + barStart, -2.7, -1.4 + barEnd, -2.0,
                        when {
                            ...
                            colorMode.equals("Astolfo", ignoreCase = true) -> RenderUtils.Astolfo2(i * distanceValue.get(), saturationValue.get(), brightnessValue.get(), astolfoRainbowOffset.get(), astolfoRainbowIndex.get(), astolfoclient.get().toFloat())
        ...
        if(char.get()){
            ...
            RainbowFontShader.begin(rainbow, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                fontRenderer.drawString(displayText, 0F, 0F, when {
                    ...
                    colorMode.equals("Astolfo", ignoreCase = true) -> RenderUtils.Astolfo2(counter[0] * 100, saturationValue.get(), brightnessValue.get(), astolfoRainbowOffset.get(), astolfoRainbowIndex.get(), astolfoclient.get().toFloat())
