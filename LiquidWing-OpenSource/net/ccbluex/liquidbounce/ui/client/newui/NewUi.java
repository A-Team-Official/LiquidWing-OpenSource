...
import net.ccbluex.liquidbounce.utils.render.animation.AnimationUtils;
...
public class NewUi extends GuiScreen {
    ...
    private void drawFullSized(int mouseX, int mouseY, float partialTicks, Color accentColor) {
            ...
            if(!got){
                try {
                    mc.getTextureManager().loadTexture(
                            ...
                            (AbstractTexture) new DynamicTexture(ImageIO.read(new URL("https://q.qlogo.cn/headimg_dl?dst_uin=" +LiquidBounce.INSTANCE.getQQ() + "&spec=640&img_type=png"))));
                    got = true;
        ...
        if (Fonts.misans35.getStringWidth(mc.player.getGameProfile().getName()) > 70)
            Fonts.misans40.drawString(Fonts.misans40.getStringWidth(LiquidBounce.INSTANCE.getUSERNAME()) + "...", 100, 78 - Fonts.misans40.getFontHeight() + 15, -1);
        else
            Fonts.misans40.drawString(LiquidBounce.INSTANCE.getUSERNAME(), 100, 78 - Fonts.misans40.getFontHeight() + 15, -1);
