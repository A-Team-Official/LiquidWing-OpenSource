...
class CategoryElement(val category: ModuleCategory): MinecraftInstance() {
    ...
    fun drawLabel(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float) {
        ...
        FontLoaders.F20.drawStringWithShadow(category.Chinese, x + 10F, y + height / 2F - FontLoaders.F20.height / 2F + 7F, -1)
        }
        else
        {
            Fonts.misans40.drawString(name, x + 10F, y + height / 2F - Fonts.misans40.fontHeight / 2F + 2F, -1)
        }
    }
    ...
    fun drawPanel(mX: Int, mY: Int, x: Float, y: Float, width: Float, height: Float, wheel: Int, accentColor: Color) {
        ...
        if (CustomUI.Chinese.get()) {
            FontLoaders.F20.drawStringWithShadow(
                "${ChatFormatting.GRAY}模块 > ${ChatFormatting.RESET}${category.Chinese}",
                x + 10F,
                y + 10F,
                -1
            )
            ...
            Fonts.misans35.drawString(
                "${ChatFormatting.GRAY}Modules > ${ChatFormatting.RESET}$name",
                x + 10F,
                y + 10F,
                -1
            )
