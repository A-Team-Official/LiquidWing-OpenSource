...
class ListElement(val saveValue: ListValue): ValueElement<String>(saveValue) {
    ...
    private val maxSubWidth = -(saveValue.values.map { -FontLoaders.F20.getStringWidth(it) }.sorted().firstOrNull() ?: 0) + 20

    companion object {
        val expanding = LiquidBounce.wrapper.classProvider.createResourceLocation("liquidwing/expand.png") }
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color, accentColor: Color): Float {
        ...
        glPopMatrix()
        glPopMatrix()
        FontLoaders.F20.drawStringWithShadow(value.get(), x + width - 14F - maxSubWidth, y + 6F, -1)
        glPushMatrix()
        ...
        if (percent > 0F) for (subV in unusedValues) {
            FontLoaders.F20.drawStringWithShadow(subV, 0F, (16F + vertHeight) * percent - 1F, Color(.5F, .5F, .5F, percent.coerceIn(0F, 1F)).rgb)
            vertHeight += 16F
        }
        glPopMatrix()
