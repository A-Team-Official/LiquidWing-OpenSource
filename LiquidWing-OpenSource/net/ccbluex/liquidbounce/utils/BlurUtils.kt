...
object BlurUtils : MinecraftInstance() {
    ...
    private val framebuffer = blurShader.mainFramebuffer
    private val frbuffer = blurShader.getFramebufferRaw("result")
    ...
    private fun setValues(strength: Float, x: Float, y: Float, w: Float, h: Float, width: Float, height: Float, force: Boolean = false) {
        ...
        for (i in 0..1) {
            blurShader.listShaders[i].shaderManager.getShaderUniform("Radius")!!.set(strength)
            blurShader.listShaders[i].shaderManager.getShaderUniform("BlurXY")!![x] = height - y - h
            blurShader.listShaders[i].shaderManager.getShaderUniform("BlurCoord")!![w] = h
        }
    }
