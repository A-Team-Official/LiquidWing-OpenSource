...
import net.ccbluex.liquidbounce.utils.rise6.RiseShaders;
...
import static ad.utils.Color.VisualUtils.*;
...
public final class RenderUtils extends MinecraftInstance {
    ...
    public static void drawShadow_arraylist(float x, float y, float width, float height) {
...
//		drawTexturedRect(x + width, y, 9, height, "panelright");
    ...
    public static void doGlScissor(float x, float y, float windowWidth2, float windowHeight2) {
        int scaleFactor = 1;
        float k = mc.getGameSettings().getGuiScale();
        if (k == 0) {
            k = 1000;
        }
        ...
        GL11.glScissor((int) (x * scaleFactor), (int) (mc.getDisplayHeight() - (y + windowHeight2) * scaleFactor),
    ...
    public static void drawCheckeredBackground(float x, float y, float x2, float y2) {
        RenderUtils.drawRect(x, y, x2, y2, RenderUtils.getColor(16777215));
        for(boolean offset = false; y < y2; ++y) {
            for(float x1 = x + (float)((offset = !offset) ? 1 : 0); x1 < x2; x1 += 2.0F) {
                if (x1 <= x2 - 1.0F) {
                    RenderUtils.drawRect(x1, y, x1 + 1.0F, y + 1.0F, RenderUtils.getColor(8421504));
                }
            }
        }
    }
    ...
    public static void renderParticles(final List<Particle> particles) {
        ...
        try {
            for (final Particle particle : particles) {
                ...
                final double distanceFromPlayer = mc.getThePlayer().getDistance(v.xCoord, v.yCoord - 1, v.zCoord);
                int quality = (int) (distanceFromPlayer * 4 + 10);

                if (quality > 350)
                    quality = 350;

                if (!isInViewFrustrum(new EntityEgg((World) mc.getTheWorld(), v.xCoord, v.yCoord, v.zCoord)))
                    draw = false;
                ...
                if (draw) {
                    ...
                    GL11.glRotated(mc.getRenderManager().getPlayerViewX(), mc.getGameSettings().getThirdPersonView() == 2 ? -1.0D : 1.0D, 0.0D, 0.0D);
    ...
    public static  float getMiddleOfBox(float boxHeight, IFontRenderer fontRenderer) {
        return boxHeight / 2f - fontRenderer.getFontHeight() / 2f;
    }
    ...
    public static void drawRect5(double x, double y, double x2, double y2, int color) {
    ...
    public static void newDrawRect2(float left, float top, float right, float bottom, int color)
    {
        if (left < right)
        {
            float i = left;
    ...
    public static void drawTexturedRect(float x, float y, float width, float height, String image, IScaledResolution sr) {
        ...
        GlStateManager.enableBlend();
        GlStateManager.disableAlpha();
        mc.getTextureManager().bindTexture((classProvider.createResourceLocation("shaders/" + image + ".png")));
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    public static void drawCircle(final IEntity entity, final double rad, final int color, final boolean shade) {
        ...
        for (float i = 0; i < Math.PI * 2; i += Math.PI * 2 / 64.F) {
            ...
            c =  ad.utils.RenderUtils.getGradientOffset(
                    new Color(CustomUI.r.get(), CustomUI.g.get(), CustomUI.b.get()),
                    new Color(CustomUI.r2.get(), CustomUI.g2.get(), CustomUI.b2.get()),i
            );
    ...
    public static void drawSmoothRect(double left, double top, double right, double bottom, int color) {
        ...
        GL11.glEnable(GL11.GL_BLEND);
        GL11.glEnable(GL11.GL_LINE_SMOOTH);
        drawRect4(left, top, right, bottom, color);
        GL11.glScalef(0.5f, 0.5f, 0.5f);
        drawRect4(left * 2.0f - 1.0f, top * 2.0f, left * 2.0f, bottom * 2.0f - 1.0f, color);
        drawRect4(left * 2.0f, top * 2.0f - 1.0f, right * 2.0f, top * 2.0f, color);
        drawRect4(right * 2.0f, top * 2.0f, right * 2.0f + 1.0f, bottom * 2.0f - 1.0f, color);
        GL11.glDisable(GL11.GL_BLEND);
        GL11.glScalef(2.0f, 2.0f, 2.0f);
    }
    ...
    public static void drawRect4(double x, double y, double x2, double y2, int color) {
        GlStateManager.color(1, 1, 1, 1);
        RenderUtils.drawRect7(x, y, x2, y2, color);
    }
    ...
    public static void drawRect7(double left, double top, double right, double bottom, int color) {
        if (left < right) {
            double i = left;
            left = right;
            right = i;
    ...
    public static void drawRoundedRect(float x, float y, float width, float height, float edgeRadius, int color, float borderWidth, int borderColor) {
        if (color == 16777215) color = ad.utils.Colors.WHITE.c;
        if (borderColor == 16777215) borderColor = ad.utils.Colors.WHITE.c;
    ...
    public static void renderNameTag(final String string, final double x, final double y, final double z) {
        ...
        final int width = Fonts.tenacitybold35.getStringWidth(string) / 2;

        drawRect(-width - 1, -1, width + 1, Fonts.tenacitybold35.getFontHeight(), Integer.MIN_VALUE);
        Fonts.tenacitybold35.drawString(string, -width, 1.5F, Color.WHITE.getRGB(), true);
